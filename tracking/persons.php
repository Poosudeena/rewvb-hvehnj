					...
					<div class="card-body">
						<table class="table table-condensed table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$types = $conn->query("SELECT *,concat(firstname) as name,concat(lastname) as name1,concat(hod_name) as name2, concat(RequiredComponents) as caddress , concat(QuantitiesRequired) as caddress1 , concat(Dept) as caddress2 , concat(RequestedSpecialLab) as caddress3 FROM persons order by name asc");
								...
								<!-- <div class="btn-group">
								  ...
								  <button class="btn btn-sm btn-outline-primary edit_establishment" type="button" data-id="<?php echo $row['id'] ?>" >Approve</button>
									    ...
									    <button class="btn btn-sm btn-outline-danger delete_persons" type="button" data-id="<?php echo $row['id'] ?>">Reject</button>
								  </div>
								</div>
								</center>	
								</td>
