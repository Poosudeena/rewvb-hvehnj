...
session_start();
if(isset($_GET['id'])){
$qry = $conn->query("SELECT t.*,concat(p.firstname) as name, concat(p.lastname) as name1, concat(p.hod_name) as name2,concat(p.RequiredComponents) as caddress, concat(p.QuantitiesRequired) as caddress1,concat(p.Dept) as caddress2,concat(p.RequestedSpecialLab) as caddress3,e.name as ename,p.tracking_id FROM person_tracks t inner join persons p on p.id = t.person_id inner join establishments e on e.id = t.establishment_id  where t.id= ".$_GET['id']);
...
<div class="container-fluid">
	<form action="" id="manage-records">
		...
		<div id="details" <?php echo isset($id) ? "style='display:block'" : 'style="display:none"' ?>>
			...
			<p><b>First Name: <span id="name"><?php echo isset($id) ? ucwords($name) : '' ?></span></b></p>
            <p><b>Last Name: <span id="name"><?php echo isset($id) ? ucwords($name1) : '' ?></span></b></p>
            <p><b>HOD: <span id="address"><?php echo isset($id) ? ucwords($name2) : '' ?></span></b></p>
            <p><b>Required Component: <span id="name"><?php echo isset($id) ? $caddress : '' ?></span></b></p>
            <p><b> Quantities Required: <span id="name"><?php echo isset($id) ?$caddress1 : '' ?></span></b></p>
            <p><b>Dept/Special lab: <span id="name"><?php echo isset($id) ? $caddress2 : '' ?></span></b></p>
            <p><b>Requested Special Lab: <span id="name"><?php echo isset($id) ? $caddress3 : '' ?></span></b></p>
