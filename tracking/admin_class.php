...
Class Action {
	...
	function save_person(){
		...
		$data .= ", RequiredComponents = '$RequiredComponents' ";
		$data .= ", QuantitiesRequired = '$QuantitiesRequired' ";
		$data .= ", Dept = '$Dept' ";
		$data .= ", RequestedSpecialLab = '$RequestedSpecialLab' ";
		// $data .= ", state = '$state' ";
	...
	function get_pdetails(){
		extract($_POST);
		$get = $this->db->query("SELECT *,concat(firstname) as name,concat(lastname) as name1,  concat(hod_name) as name2, concat(RequiredComponents) as caddress, concat(QuantitiesRequired) as caddress1, concat(Dept) as caddress2, concat(RequestedSpecialLab) as caddress3 FROM persons where tracking_id = $tracking_id ");
		$data = array();
		if($get->num_rows > 0){
